/**
 * 이 파일은 iconsAsset/convert.js에 의해 자동 생성되었습니다.
 * 직접 수정하는 대신 iconsAsset/convert.js를 수정하세요.
 */

import { memo, forwardRef } from 'react';

import { IconBase } from '../icon.base';
import { IconProps } from '../icon.type';

export const IcArrowSquareDownRightLine = memo(
  forwardRef<SVGSVGElement, IconProps>((props, ref) => (
    <IconBase ref={ref} viewBox="0 0 24 24" {...props}>
      <path
        d="M19.7301 16.31C19.6921 16.2208 19.6378 16.1394 19.5701 16.07L16.6401 13.14C16.4531 12.9394 16.1715 12.8568 15.9058 12.9246C15.64 12.9925 15.4325 13.2 15.3647 13.4657C15.2968 13.7315 15.3794 14.0131 15.5801 14.2L17.2301 15.85H10.1401C8.0966 15.85 6.44005 14.1935 6.44005 12.15V10C6.42255 8.00019 8.00137 6.35041 10.0001 6.28003H17.9001C18.3143 6.28003 18.6501 5.94424 18.6501 5.53003C18.6501 5.11582 18.3143 4.78003 17.9001 4.78003H10.0001C7.20197 4.87729 4.98755 7.18029 5.00005 9.98003V12.15C5.00556 15.0196 7.33045 17.3445 10.2001 17.35H17.2901L15.5801 19C15.2876 19.2928 15.2876 19.7672 15.5801 20.06C15.7199 20.2017 15.911 20.281 16.1101 20.28C16.3093 20.2821 16.5008 20.2026 16.6401 20.06L19.5701 17.13C19.6378 17.0607 19.6921 16.9793 19.7301 16.89C19.8102 16.705 19.8102 16.4951 19.7301 16.31Z"
        xmlns="http://www.w3.org/2000/svg"
      />
    </IconBase>
  ))
);

IcArrowSquareDownRightLine.displayName = 'IcArrowSquareDownRightLine';
